...
defs  = [] # define macros
files = []
install_dir = sysconfig.get_python_lib() + "/pynel"

# Check for Python Xlib
try:
    from Xlib import X, display, Xatom, Xutil
except:
    print "\nPynel requires the Python X library -"
...
for config in configs:
    ...
    if os.path.isfile(config):
        ...
        if package == "imlib2-config":
            print "\nPynel requires the Imlib2 library -"
...
if len(sys.argv) > 1 and sys.argv[1] != "sdist":
    ...
    if ("IMLIB2_FIX", 1) in defs:
        for line in fileinput.input(["cpynel.c"], inplace=1):
